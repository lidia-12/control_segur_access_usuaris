...
def take_pic(nom_foto="foto_capturada.jpg"):
    ...
    # Inicia la captura de vídeo de la càmera especificada
    cap = cv2.VideoCapture('/dev/video0')

    # Comença fils per mostrar text i preparar la càmera
    ...
    # Reproduïx una melodia per indicar que s'ha fet una foto
    buzzer.select_song("Foto")

    if ret:
        # Guarda la imatge capturada amb el nom especificat
        cv2.imwrite(nom_foto, frame)
        # Mostra la imatge en el display en un fil separat
        threading.Thread(target=show_pic_in_background, args=(nom_foto,)).start()
        print("Enviant foto")
        # Envía la foto al servidor en el fil principal i obté una resposta
        respostaServerPIC = sendToServerPIC(nom_foto)
...
#        os.remove(nom_foto)
    ...
    # Assegura que la captura es tanqui independentment del resultat
    cap.release()
